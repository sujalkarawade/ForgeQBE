...
export default function ExampleEditor({ onResult, onLoading, loading }) {
  ...
  const [inputMode, setInputMode] = useState('table'); // 'table' | 'json'
  ...
  // Get all unique column names across examples
  const getColumns = () => {
    ...
    <div className="example-editor">
      <div className="editor-header">
        <div className="editor-header-left">
          ...
          <p className="editor-desc">Enter rows that represent the data you want to find</p>
        </div>
      ...
      {tables.length > 0 && inputMode === 'table' && (
        <div className="table-selector">
          <label htmlFor="table-select">Target table (optional):</label>
          <select
            id="table-select"
            value={selectedTable}
      ...
      <div className="hint-section">
        <label htmlFor="hint-input" className="hint-label">
          ...
          Natural language hint (optional)
        </label>
        <input
          id="hint-input"
          type="text"
      ...
      >
        {loading ? (
          <><span className="spinner-btn" /> Analyzing &amp; Generating SQL…</>
        ) : (
